...
#%%
question_responses = [dict(item=q['id'], annotator=k, text_idx=text_idx, condition=text_key.condition.iloc[text_idx], response=response)
    for k, v in by_annotator.items()
    for q in v['questions']
    for text_idx, response in enumerate(q['responses'])
    if response is not None]
pd.DataFrame(question_responses).to_csv('analysis')
#%%
